...
final class ProjectController {
    ...
    func exportPNG() async {
        ...
        catch { await showError(L10n.t("Couldn’t export PNG"), error: error) }
    }
    ...
    func canvasSize() async {
        ...
        let options: CanvasSizeOptions? = await withCheckedContinuation { continuation in
            ...
            sheet.title = L10n.t("Canvas Size")
        ...
        do {
            ...
            session.applyDocumentSize(resized, actionName: L10n.t("Canvas Size"))
        } catch { await showError(L10n.t("Couldn’t change canvas size"), error: error) }
    }
    ...
    func imageSize() async {
        ...
        } catch { await showError(L10n.t("Couldn’t resize the image"), error: error) }
    }
    ...
    func exportJPEG() async {
        ...
        do {
            ...
            let data: Data? = await withCheckedContinuation { continuation in
                let sheet = NSWindow()
                sheet.styleMask = [.titled, .fullSizeContentView]
                sheet.title = L10n.t("Export JPEG")
        ...
        } catch { await showError(L10n.t("Couldn’t export JPEG"), error: error) }
    }
    ...
    private func saveCurrent(asNew: Bool = false) async -> Bool {
        ...
        if destination == nil {
            ...
            panel.title = asNew ? L10n.t("Save Project As") : L10n.t("Save Project")
            let response: NSApplication.ModalResponse
            if let window { response = await panel.beginSheetModal(for: window) }
            else { response = await panel.begin() }
        ...
        do {
            ...
            await showError(L10n.t("Couldn’t save the project"), error: error)
            return false
        }
    }
    ...
    func open(_ suppliedURL: URL? = nil) async -> Bool {
        ...
        if source == nil {
            ...
            panel.title = L10n.t("Open Project")
            let response: NSApplication.ModalResponse
            if let window { response = await panel.beginSheetModal(for: window) }
            else { response = await panel.begin() }
            guard response == .OK, let url = panel.url else { return false }
        ...
        do {
            ...
            await showError(L10n.t("Couldn’t open the project"), error: error)
            return false
        }
    }
    ...
    private func confirmReplacement() async -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = L10n.format("Save changes to %@?", session.projectURL?.lastPathComponent ?? L10n.t("Untitled"))
