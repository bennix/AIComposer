        ...
        HStack(spacing: 12) {
            ...
            if session.tool == .brush {
                Picker("Mode", selection: $session.brushMode) {
                    ForEach(BrushToolMode.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
            ...
            if session.tool == .blur {
                Picker("Mode", selection: $session.blurMode) {
                    ForEach(BlurToolMode.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented).labelsHidden().fixedSize()
            ...
            if session.tool == .spotHealing {
                Picker("Type", selection: $session.spotHealingMode) {
                    ForEach(SpotHealingMode.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
