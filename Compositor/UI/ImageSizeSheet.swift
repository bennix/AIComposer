...
struct ImageSizeSheet: View {
    ...
    @ViewBuilder private var sheet: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Text(L10n.format("Current: %d × %d pixels", document.width, document.height)).foregroundStyle(.secondary)
            Picker("Units", selection: $unit) {
                ForEach(units.filter { resample || ($0 != "Pixels" && $0 != "Percent") }, id: \.self) { Text(L10n.t($0)) }
            }
            HStack {
            ...
            if resample {
                Picker("Sampling", selection: $sampling) {
                    ForEach(LayerSampling.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
            ...
            Text(valid ? L10n.format("Result: %d × %d pixels", Int(width.rounded()), Int(height.rounded())) : L10n.t("Use 1–30,000 pixels per side, up to 100 megapixels, and 1–9,600 pixels/inch."))
