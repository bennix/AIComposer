    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Picker("Channel", selection: $settings.channel) {
                ForEach(LevelsChannel.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
            ...
            HStack {
                if let selected, points.indices.contains(selected) {
                    Text(L10n.format("Input %d · Output %d", Int(points[selected].x), Int(points[selected].y))).monospacedDigit()
                }
                Spacer()
                Button("Remove point") {
