...
struct LassoControls: View {
        ...
        HStack(spacing: 12) {
            ...
            if session.tool == .marquee {
                Picker("Shape", selection: Binding(get: { session.marqueeKind }, set: { kind in
                    ...
                    ForEach(LassoKind.marqueeChoices, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
            ...
            if session.tool == .lasso {
                Picker("Lasso", selection: Binding(get: { session.lassoKind }, set: { kind in
                    ...
                    ForEach(LassoKind.lassoChoices, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented).labelsHidden().fixedSize()
            ...
            Picker("Mode", selection: Binding(get: { session.displayedSelectionMode },
                ...
                ForEach(SelectionMode.allCases, id: \.self) { Text(L10n.t($0.rawValue)).tag($0) }
            }
            .pickerStyle(.segmented).labelsHidden().fixedSize()
    ...
    private func modifyControl(_ title: String, amount: Binding<Int>, action: @escaping () -> Void) -> some View {
        HStack(spacing: 6) {
            Button(L10n.t(title), action: action)
            TextField(L10n.t(title), value: Binding(get: { amount.wrappedValue },
        ...
        .help(L10n.format("%@ the selection by this many pixels", title))
    }
}
