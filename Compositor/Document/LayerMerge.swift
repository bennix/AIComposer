...
extension EditorSession {
    ...
    private func mergePlan() -> (ids: [UUID], removed: Set<UUID>, name: String, parent: UUID?, anchor: UUID, action: String)? {
        ...
        if selectedLayerIDs.count > 1 {
            ...
            return (ordered.map(\.id), picked, top.name, top.parentID, top.id, L10n.t("Merge Layers"))
        }
        if active.isGroup {
            ...
            return (ids, Set(ids), active.name, active.parentID, active.id, L10n.t("Merge Group"))
        }
        ...
        return ([below.id, active.id], [below.id, active.id], below.name, active.parentID, active.id, L10n.t("Merge Down"))
    }

    var canMergeLayers: Bool { mergePlan() != nil }
    var mergeTitle: String { mergePlan()?.action ?? L10n.t("Merge Down") }
