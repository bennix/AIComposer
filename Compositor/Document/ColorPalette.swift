...
enum ColorPickerTarget: Equatable {
    ...
    var title: String {
        switch self {
        case .text: return L10n.t("Color Picker (Text Color)")
        case .effect(let kind): return L10n.format("Color Picker (%@ Color)", kind.rawValue)
        case .palette(let background): return background ? L10n.t("Color Picker (Background Color)") : L10n.t("Color Picker (Foreground Color)")
        case .gradientMap(let highlights): return highlights ? L10n.t("Color Picker (Gradient Map Highlights)") : L10n.t("Color Picker (Gradient Map Shadows)")
        }
    }
}
